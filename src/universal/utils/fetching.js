...
export const fetchGraphQL = async graphParams => {
  ...
  const authToken = localStorage && localStorage.getItem(socketOptions.authTokenName);
  const currentHostUrl = hostUrl();
  const graphQLUrl = basename ? `${currentHostUrl}/${basename}/graphql` : `${currentHostUrl}/graphql`;
